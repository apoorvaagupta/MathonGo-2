...
router.post('/withFilters', function (req, res) {
    ...
    let classArray = req.body.filter.hasOwnProperty('classObject') ? req.body.filter.classObject : [0],
        subjectArray = req.body.filter.hasOwnProperty('subjectObject') ? req.body.filter.subjectObject : [0],
        categoryArray = req.body.filter.hasOwnProperty('categoryObject') ? req.body.filter.categoryObject : [0],
        difficultyArray = req.body.filter.hasOwnProperty('difficultyObject') ? req.body.filter.difficultyObject : [''],
        mediumArray = req.body.filter.hasOwnProperty('mediumObject') ? req.body.filter.mediumObject : [''];

    for (let i = 0; i < classArray.length; i++) {
        classArray[i] = parseInt(classArray[i]);
    }

    for (let i = 0; i < subjectArray.length; i++) {
        subjectArray[i] = parseInt(subjectArray[i]);
    }

    for (let i = 0; i < categoryArray.length; i++) {
        categoryArray[i] = parseInt(categoryArray[i]);
    }
